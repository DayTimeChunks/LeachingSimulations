...
def pesti_ret(Kd_sterile, Kd_untreat,
              pb,
              ov_sat,
              water_data,
              area, soil_height,
    ...
    r_factor_sterile = 1 + (pb * Kd_sterile) / ov_sat
    ...
    for i in range(3):
        ...
        for t in range(len(cum_time_30min)):
            ...
            conc_new = conc_old*exp(-(leached_vol[i][t]) / (r_factor_sterile*vol_h2o_sat))
    ...
    r_factor_untreat = 1 + (pb * Kd_untreat) / ov_sat
    for i in range(3):
        mass_left_old = conc_old * vol_h2o_sat
        cum_mass_out = 0
        cum_mass_out_dt = []
        ...
        for t in range(len(cum_time_30min)):
            #  water_data[:, t] = leached volume at time-step t
            conc_new = conc_old * exp(-(leached_vol[i][t]) / (r_factor_untreat * vol_h2o_sat))
            mass_left_new = conc_new * vol_h2o_sat
            mass_out = mass_left_old - mass_left_new
            cum_mass_out += mass_out
            mass_out_dt.append(mass_out)
